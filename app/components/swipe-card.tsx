...
export function SwipeDeck({ initialCard }: { initialCard: QueueCard | null }) {
  ...
  const loadNext = useCallback(async () => {
    ...
    try {
      ...
      setError('Could not load the next name — check your connection and try again.')
      setCard(null)
    }
  }, [])
  ...
  if (!card) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-2 p-8 text-center">
        ...
        <Text>No new names right now — check back later, or add one yourself on &ldquo;My Names&rdquo;.</Text>
      </div>
    )
  }
...
function SwipeCard({ card, onAct }: { card: QueueCard; onAct: (action: SwipeAction) => void }) {
    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-6 p-4">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3 py-6 text-center">
          ...
          <Badge color={card.gender === 'f' ? 'pink' : 'blue'}>{card.gender === 'f' ? 'Girl' : 'Boy'}</Badge>

          {card.partnerAction && (
            <Badge color="violet">Your partner {card.partnerAction === 'love' ? 'loved' : 'shortlisted'} this</Badge>
          )}
          ...
          {card.meaning && (
            <Text>
              ...
              {card.meaningUrl && (
                <>
                  {' '}
                  <TextLink href={card.meaningUrl} target="_blank" rel="noreferrer">
                    (via Wikipedia)
                  </TextLink>
                </>
              )}
      ...
      <Text className="text-xs">Swipe left to veto, right to shortlist, up to love — or use the buttons.</Text>
    </div>
  )
}
